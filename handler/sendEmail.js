...
const getSubs = async() => {
    const subs = await axios.get("urltogetsubscribers");
    let list = subs.data.map(sub=> {
        return sub.email;
    });
    return list;
};
...
const getQuote = async () => {
    const getQuotes = await axios.get("urlofgetquoteapi");
    let len = getQuote.data.quotes.length;
    let randomQuote = getQuotes.data.quotes[Math.floor(Math.random()* len)];
    return randomQuote;
};
